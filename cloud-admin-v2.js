...
if(!cfg.SUPABASE_PUBLISHABLE_KEY&&cfg.CONFIG_ENDPOINT){
  try{const r=await fetch(cfg.CONFIG_ENDPOINT,{cache:'no-store'});if(r.ok)cfg={...cfg,...await r.json()}}catch(e){console.warn('JPro admin config',e)}
}
...
async function context(){
  const companyId=window.JPRO_CLOUD?.companyId,userId=window.JPRO_CLOUD?.userId;
  if(!sb||!companyId||!userId)return null;
  const [{data:company,error:cErr},{data:member,error:mErr}]=await Promise.all([
    sb.from('companies').select('id,name,join_code').eq('id',companyId).maybeSingle(),
    sb.from('company_memberships').select('role,active').eq('company_id',companyId).eq('user_id',userId).maybeSingle()
  ]);
  if(cErr)throw cErr;if(mErr)throw mErr;
  if(!company||!member?.active)return null;
  return{company,user:{id:userId},role:member.role||''};
}

async function openMembers(){
  try{
    const ctx=await context();
    if(!ctx)return alert('ابتدا وارد یک شرکت شوید.');
    ...
    m.id='jproMembersModal';
    m.style.cssText='position:fixed;inset:0;z-index:4000;background:rgba(15,35,59,.42);display:grid;place-items:center;padding:18px;font-family:Tahoma,Arial,sans-serif';
    ...
    document.getElementById('jmClose').onclick=()=>m.remove();
