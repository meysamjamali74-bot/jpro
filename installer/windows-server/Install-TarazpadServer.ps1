...
function Find-FreePort([int]$start,[int]$count=31){for($p=$start;$p -lt ($start+$count);$p++){if(Test-FreePort $p){return $p}};throw "No free local port found in range $start-$($start+$count-1)."}
function Test-TcpConnection([string]$serverHost,[int]$port,[int]$timeoutMs=1000){
  $client=$null
  try{$client=New-Object System.Net.Sockets.TcpClient;$ar=$client.BeginConnect($serverHost,$port,$null,$null);if(-not $ar.AsyncWaitHandle.WaitOne($timeoutMs,$false)){return $false};$client.EndConnect($ar);return $client.Connected}catch{return $false}finally{if($client){$client.Close()}}
}
...
function Test-TarazpadHealth([int]$port){try{$r=Invoke-RestMethod "http://127.0.0.1:$port/api/health" -TimeoutSec 2;return [bool]$r.ok}catch{return $false}}
function Resolve-WebPort($existingConfig){
  if($existingConfig -and $existingConfig.webPort){
    $existingPort=[int]$existingConfig.webPort
    if((Test-TarazpadHealth $existingPort) -or (Test-FreePort $existingPort)){Ok "Existing Tarazpad web port $existingPort preserved.";return $existingPort}
    Warn "Configured web port $existingPort is occupied by another process; selecting a safe replacement."
  }
  $p=Find-FreePort $DefaultWebPort 64
  if($p -ne $DefaultWebPort){Warn "Default web port $DefaultWebPort is occupied; Tarazpad will use $p."}else{Ok "Default web port $DefaultWebPort is available."}
  return $p
}
function Invoke-MySqlProbe([string]$mysqlExe,[int]$port,[string]$user,[string]$password){
  $hadPwd=Test-Path Env:MYSQL_PWD;$oldPwd=$env:MYSQL_PWD
  try{$env:MYSQL_PWD=$password;& $mysqlExe --protocol=tcp --host=127.0.0.1 --port=$port --user=$user --batch --skip-column-names --execute='SELECT 1;' 2>$null | Out-Null;return ($LASTEXITCODE -eq 0)}catch{return $false}finally{if($hadPwd){$env:MYSQL_PWD=$oldPwd}else{Remove-Item Env:MYSQL_PWD -ErrorAction SilentlyContinue}}
}

function Ensure-VCRuntime{
  foreach($key in @('HKLM:\SOFTWARE\Microsoft\VisualStudio\14.0\VC\Runtimes\x64','HKLM:\SOFTWARE\WOW6432Node\Microsoft\VisualStudio\14.0\VC\Runtimes\x64')){try{$r=Get-ItemProperty $key -ErrorAction Stop;if($r.Installed -eq 1){Ok "Microsoft Visual C++ Runtime already installed (v$($r.Major).$($r.Minor)); skipped.";return}}catch{}}
  ...
  Step 'Installing missing Microsoft Visual C++ runtime only...';$p=Start-Process $installer -ArgumentList '/install','/quiet','/norestart' -Wait -PassThru
...
function Test-MySqlBinary([string]$mysqlExe){
  ...
  try{$v=& $mysqlExe --version 2>$null;if($v -match 'Ver\s+(\d+)\.(\d+)\.(\d+)' -and [int]$Matches[1] -eq 8 -and [int]$Matches[2] -eq 4){$bin=Split-Path -Parent $mysqlExe;$base=Split-Path -Parent $bin;$mysqld=Join-Path $bin 'mysqld.exe';$dump=Join-Path $bin 'mysqldump.exe';if((Test-Path $mysqld)-and(Test-Path $dump)){return [pscustomobject]@{Mysql=$mysqlExe;Mysqld=$mysqld;Dump=$dump;Base=$base;Version="$($Matches[1]).$($Matches[2]).$($Matches[3])";Source='system'}}}}catch{};return $null
}
function Find-CompatibleMySql{
  $paths=New-Object System.Collections.Generic.List[string];$cmd=Get-Command mysql.exe -ErrorAction SilentlyContinue;if($cmd){[void]$paths.Add($cmd.Source)}
  ...
  foreach($p in ($paths|Select-Object -Unique)){$found=Test-MySqlBinary $p;if($found){return $found}};return $null
}
function Expand-BundledMySql{
  ...
  $tmp=Join-Path $env:TEMP ('tarazpad-mysql-'+[guid]::NewGuid());New-Item $tmp -ItemType Directory -Force|Out-Null;Step 'No compatible MySQL 8.4 detected; extracting bundled MySQL 8.4.11...'
  $tar=Get-Command tar.exe -ErrorAction SilentlyContinue;if($tar){& $tar.Source -xf $zip -C $tmp;if($LASTEXITCODE -ne 0){throw "MySQL extraction failed with code $LASTEXITCODE."}}else{Expand-Archive $zip -DestinationPath $tmp -Force}
  $top=Get-ChildItem $tmp -Directory|Select-Object -First 1;if(!$top){throw 'Invalid MySQL archive structure.'};Copy-Item (Join-Path $top.FullName '*') $MySqlRuntimeLocal -Recurse -Force;Remove-Item $tmp -Recurse -Force
...
function Repair-ExistingDatabaseCredential($existing){
  if(Invoke-MySqlProbe $existing.mysqlExe ([int]$existing.mysqlPort) $existing.mysqlUser $existing.mysqlPassword){Ok 'Existing Tarazpad database credential validated.';return $existing}
  Warn 'Saved Tarazpad application database credential is invalid; attempting safe credential recovery.'
...
function Ensure-MySql{
  Ensure-VCRuntime;$existing=$null;if(Test-Path $ConfigPath){try{$existing=Get-Content $ConfigPath -Raw|ConvertFrom-Json}catch{Warn 'Existing server.json is unreadable; installer will not overwrite initialized business data blindly.'}}
  ...
  if($service -and $existing -and (Test-Path $existing.mysqlExe)){
    ...
    if(!(Wait-Tcp ([int]$existing.mysqlPort) 30)){throw "Existing Tarazpad MySQL did not open configured port $($existing.mysqlPort). See $LogRoot\mysql-error.log"}
    $existing=Repair-ExistingDatabaseCredential $existing
  ...
  if((Test-Path (Join-Path $MySqlData 'mysql')) -and (!$existing -or !$service)){throw 'Existing Tarazpad MySQL data was found without a complete valid service/configuration. Installer stopped to protect business data; no database files were deleted or reinitialized.'}
  $mysql=Find-CompatibleMySql;if($mysql){Ok "Compatible system MySQL $($mysql.Version) detected; installation skipped and existing binaries will be reused."}else{$mysql=Expand-BundledMySql}
  New-Item $MySqlData -ItemType Directory -Force|Out-Null;$dbPort=Find-FreePort 3307 31;Write-MySqlConfig $mysql $dbPort
  ...
  $service=Get-Service $MySqlService;if($service.Status -ne 'Running'){Start-Service $MySqlService;$service.WaitForStatus('Running','00:00:30')};if(!(Wait-Tcp $dbPort 60)){throw "Tarazpad MySQL service did not open TCP port $dbPort. See $LogRoot\mysql-error.log"};Ok "Tarazpad MySQL ready on local port $dbPort."
...
FLUSH PRIVILEGES;
"@
  $temp=Join-Path $env:TEMP ('tarazpad-db-'+[guid]::NewGuid()+'.sql');$sql|Set-Content $temp -Encoding utf8;$env:MYSQL_PWD='';Get-Content $temp|& $mysql.Mysql --protocol=tcp --host=127.0.0.1 --port=$dbPort --user=root --skip-password
...
Get-NetFirewallRule -DisplayName 'Tarazpad ERP Web' -ErrorAction SilentlyContinue|Remove-NetFirewallRule -ErrorAction SilentlyContinue;New-NetFirewallRule -DisplayName 'Tarazpad ERP Web' -Direction Inbound -Action Allow -Protocol TCP -LocalPort $WebPort -Profile Domain,Private|Out-Null;Ok "Private/domain firewall rule configured for port $WebPort."
Step 'Running application health check...';$healthy=$false
for($i=0;$i -lt 60;$i++){try{$r=Invoke-RestMethod "http://127.0.0.1:$WebPort/api/health" -TimeoutSec 2;if($r.ok){$healthy=$true;break}}catch{};Start-Sleep 1};if(!$healthy){throw "Tarazpad API did not become healthy on port $WebPort. See $LogRoot"};Ok 'Tarazpad API and MySQL health check passed.'
