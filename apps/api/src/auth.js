...
function loadJwtSecret(){
  ...
  if(secret && secret.length>=48)return secret;
  if(process.env.NODE_ENV==='production')throw new Error('JWT_SECRET must be configured with at least 48 characters in production.');
  return secret||'dev-only-change-me';
}
const JWT_SECRET=loadJwtSecret();
const JWT_VERIFY={issuer:'tarazpad',audience:'tarazpad-client',algorithms:['HS256']};
export async function authenticate(email,password){const [rows]=await pool.execute(`SELECT u.id,u.company_id,u.full_name,u.email,u.password_hash,u.is_active,GROUP_CONCAT(r.code ORDER BY r.code) roles FROM users u LEFT JOIN user_roles ur ON ur.user_id=u.id LEFT JOIN roles r ON r.id=ur.role_id WHERE u.email=? GROUP BY u.id`,[email]);const user=rows[0];if(!user||!user.is_active||!(await bcrypt.compare(password,user.password_hash)))return null;const roles=user.roles?user.roles.split(','):[];const payload={sub:String(user.id),companyId:user.company_id,roles,name:user.full_name,email:user.email};return{token:jwt.sign(payload,JWT_SECRET,{expiresIn:'8h',issuer:JWT_VERIFY.issuer,audience:JWT_VERIFY.audience,algorithm:'HS256'}),user:payload}}
export function requireAuth(req,res,next){const v=req.headers.authorization||'';const token=v.startsWith('Bearer ')?v.slice(7):null;if(!token)return res.status(401).json({error:'AUTH_REQUIRED'});try{req.user=jwt.verify(token,JWT_SECRET,JWT_VERIFY);next()}catch{res.status(401).json({error:'INVALID_TOKEN'})}}
