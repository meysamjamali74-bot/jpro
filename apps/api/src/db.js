...
// Keep SQL DATE values as canonical YYYY-MM-DD strings. This avoids timezone-dependent
// Date coercion (and invalid `${Date}T00:00:00Z` constructions) across Jalali fiscal,
// compliance and reporting flows while leaving DATETIME/TIMESTAMP behavior unchanged.
...
function normalizeMigrationSql(file,sql){let out=String(sql);if(file==='028_platform_phase15.sql'){out=out.replaceAll('REFERENCES distribution_trips(id)','REFERENCES trips(id)')}return out}
export async function migrate(){const conn=await pool.getConnection();try{await conn.query(`CREATE TABLE IF NOT EXISTS schema_migrations (version VARCHAR(255) PRIMARY KEY,applied_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci`);const dir=path.join(rootDir,'database');const files=(await fs.readdir(dir)).filter(f=>/^\d+_.*\.sql$/.test(f)).sort();for(const file of files){const [rows]=await conn.execute('SELECT version FROM schema_migrations WHERE version=?',[file]);if(rows.length)continue;const raw=await fs.readFile(path.join(dir,file),'utf8');const sql=normalizeMigrationSql(file,raw);const statements=parseSqlScript(sql);await conn.beginTransaction();try{for(const statement of statements)await conn.query(statement);await conn.execute('INSERT INTO schema_migrations(version) VALUES (?)',[file]);await conn.commit()}catch(error){await conn.rollback();throw new Error(`Migration ${file} failed: ${error.message}`)}}}finally{conn.release()}}
export async function healthCheck(){const [rows]=await pool.query('SELECT 1 AS ok');return rows[0]?.ok===1}
export { parseSqlScript,normalizeMigrationSql };
