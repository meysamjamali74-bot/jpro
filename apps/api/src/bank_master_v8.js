...
export function registerBankMasterV8Routes(app){
  ...
  app.post('/api/iran/treasury/bank-accounts',write,wrap(async(req,res)=>{
    ...
    const [r]=await pool.execute(`INSERT INTO bank_accounts(company_id,branch_id,code,bank_name,branch_name,branch_code,account_type,account_title,account_holder,account_no,iban,card_no,card_expiry,bank_phone,bank_fax,bank_address,description,currency,opening_balance,gl_account_id,show_on_sales_invoice,show_on_receipt,is_default,is_active) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,[c,b.branchId||null,txt(b.code),txt(b.bankName),txt(b.branchName)||null,txt(b.branchCode)||null,txt(b.accountType)||null,txt(b.accountTitle),txt(b.accountHolder)||null,txt(b.accountNo)||null,v.iban,v.card,txt(b.cardExpiry)||null,txt(b.bankPhone)||null,txt(b.bankFax)||null,txt(b.bankAddress)||null,txt(b.description)||null,txt(b.currency)||'IRR',num(b.openingBalance),b.glAccountId||null,b.showOnSalesInvoice?1:0,b.showOnReceipt?1:0,b.isDefault?1:0,b.isActive===false?0:1]);
  ...
  app.put('/api/iran/treasury/bank-accounts/:id',write,wrap(async(req,res)=>{
    ...
    await pool.execute(`UPDATE bank_accounts SET branch_id=?,code=?,bank_name=?,branch_name=?,branch_code=?,account_type=?,account_title=?,account_holder=?,account_no=?,iban=?,card_no=?,card_expiry=?,bank_phone=?,bank_fax=?,bank_address=?,description=?,currency=?,opening_balance=?,gl_account_id=?,show_on_sales_invoice=?,show_on_receipt=?,is_default=?,is_active=? WHERE id=? AND company_id=?`,[b.branchId===undefined?old.branch_id:(b.branchId||null),txt(b.code??old.code),txt(b.bankName??old.bank_name),b.branchName===undefined?old.branch_name:(txt(b.branchName)||null),b.branchCode===undefined?old.branch_code:(txt(b.branchCode)||null),b.accountType===undefined?old.account_type:(txt(b.accountType)||null),txt(b.accountTitle??old.account_title),b.accountHolder===undefined?old.account_holder:(txt(b.accountHolder)||null),b.accountNo===undefined?old.account_no:(txt(b.accountNo)||null),v.iban,v.card,b.cardExpiry===undefined?old.card_expiry:(txt(b.cardExpiry)||null),b.bankPhone===undefined?old.bank_phone:(txt(b.bankPhone)||null),b.bankFax===undefined?old.bank_fax:(txt(b.bankFax)||null),b.bankAddress===undefined?old.bank_address:(txt(b.bankAddress)||null),b.description===undefined?old.description:(txt(b.description)||null),txt(b.currency??old.currency)||'IRR',b.openingBalance===undefined?old.opening_balance:num(b.openingBalance),b.glAccountId===undefined?old.gl_account_id:(b.glAccountId||null),b.showOnSalesInvoice===undefined?old.show_on_sales_invoice:(b.showOnSalesInvoice?1:0),b.showOnReceipt===undefined?old.show_on_receipt:(b.showOnReceipt?1:0),b.isDefault===undefined?old.is_default:(b.isDefault?1:0),b.isActive===undefined?old.is_active:(b.isActive?1:0),id,c]);
